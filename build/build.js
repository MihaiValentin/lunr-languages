...
for (var i = 0; i < list.length; i++) {
  ...
  if (fromTemplate) {
    data = fs.readFileSync(
      "build/snowball-js/stemmer/src/ext/" + list[i].file,
      "utf8"
    );
    ...
    f = f.replace(
      /\{\{languageName\}\}/g,
      list[i].file.replace(/Stemmer\.js/g, "")
    );
    f = f.replace(/\{\{wordCharacters\}\}/g, list[i].wordCharacters);

    f = f.replace(
      /\{\{consoleWarning\}\}/g,
      list[i].warningMessage
  ...
  // write the full file
  fs.writeFileSync(
    "lunr." + list[i].locale + ".js",
    beautify(f, { indent_size: 2 })
  );
  // and the minified version
  fs.writeFileSync(
    "min/lunr." + list[i].locale + ".min.js",
    fromTemplate
      ? cm.replace(
          /\{\{languageName\}\}/g,
          list[i].file.replace(/Stemmer\.js/g, "")
